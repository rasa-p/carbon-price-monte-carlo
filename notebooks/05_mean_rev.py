...
# ----------------------------------------------------------------------------------------------------
# Comparing confidence levels fro GBM vs OU simulations
...
print(f"{'P(loss)':<15}{np.mean(gbm_pnl < 0)*100:>15.1f}%{np.mean(ou_pnl < 0)*100:>22.1f}%") # proportion of 10,000 simulations that end below starting price (loss not profit made)
print("-" * 70)
...
plt.axvline(x = np.mean(gbm_pnl), color = 'blue', linewidth = 1, label=f'GBM mean={np.mean(gbm_pnl):.1f}')
plt.axvline(x = np.mean(ou_pnl), color = 'orange', linewidth = 1, label=f'OU mean={np.mean(ou_pnl):.1f}')
...
# -------------------------PNL histogram conclusions----------------
# SHAPE:
#   - the mean-reverting histogram seems almost perfectly symmetrical, unlike the GBM model which is positively skewed
#           --> this is expected since the GBM model has a floor limit of 0, but the profit is boundless, so the right tail extends while the left tail is bounded at -S_0
#   - the mean-reverting model has a restoring force that acts equally on both sides, so we get a symmetrical distribution
#   - this all comes back to the fact that GBM is LOGNORMAL while OU is NORMAL
#
# PEAK:
#  --> the peak value is the single most likely outcome - the mode
#   - both distributions have a peak < 0 meaning that both models predict that a small loss is most likely over 1 year
#   - the mean final price of the GBM model = 78.95 EUR while mean-reverting model = 75.44 EUR
#           --> this is due to the extreme profit values in the long right tail increasing the mean
